...
export default function MainNavbar() {
    ...
    const changeLanguage = (lang) => {
        ...
        if (currentPath.startsWith('contacts')) {
            const category = searchParams.get('category') || 'contact-us';
            navigate(`/${lang}/contacts?category=${category}`);
        } else {
    ...
    return (
      <>
          {loading && (
            <div className="fixed inset-0 flex items-center justify-center bg-white bg-opacity-75 z-1">
                <div className="animate-spin h-10 w-10 border-4 border-blue-500 border-t-transparent rounded-full"></div>
            </div>
          )}
          <nav id="topnav" className={`z-10 navbar nav-sticky bg-blue-luzenti ${initialLoad ? 'navbar-animated bg-blue-luzenti' : ''} ${navbarSticky ? "nav-sticky bg-blue-luzenti" : "bg-blue-luzenti"}`}>
              <div className="relative">
                  <div className='hidden lg:flex xl:absolute ml-10 logo'>
                      <div className='relative'>
                          <button onClick={toggleLanguageDropdown} className='text-white pt-1 text-[14px] align-middle rounded'>
                              {language === 'en' ? 'International (English)' : 'Indonesia (ID)'}
                              <span className='ml-0'>{isLanguageDropdownOpen ?  <KeyboardArrowDownIcon/> : <KeyboardArrowRightIcon/> }</span>
                          </button>
                  ...
                  <div id='navigation' style={{ display: isMenu ? 'block' : 'none' }}>
                      <ul className='navigation-menu bg-blue-luzenti lg:bg-transparent lg:gap-5 xl:gap-16'>
                          ...
                          <li className={`${["Blogs","FAQ", "blogs","Testimonies", "Testimoni"].includes(manu)? "active" : ""} has-submenu text-white text-[18px] parent-menu-item font-medium`}>
                              <Link className={`pr-4 hover:text-white ${navbarText ? "text-white" : "text-white"} text-[18px] font-medium text-center`} to="#">
                                  {t('navbar.files')}
                              </Link>
